...
**为什么需要**：P0–P6 期间全程在工作区进行、**没有提交**——HEAD 是重构前的 `ca1cdf9`（v0.9.3），
13 个已跟踪文件带着 700+ 行改动挂在工作区，另有 81 个新文件未跟踪。
一次 `git checkout -- .` / `git stash` / `git reset --hard` 就会静默毁掉已跟踪的那部分，
而新文件还在 → 工作区变成"半重构"状态，既跑不起来也说不清丢了什么。

> **现状（2026-09-16，决策 D1 之后）**：重构与后续修复**都已提交**在 `refactor/p0-p6` 分支上，
> 因此**回滚粒度 = 提交**，快照不再承担"唯一救命稻草"的角色。
> 它仍是通用工具：任何"未提交状态下做大改"的场合都能给出可校验的还原点。

```powershell
node .p6-cutover/snapshot.mjs --create                 # 生成 data/backup-p0p6-<stamp>/
node .p6-cutover/snapshot.mjs --verify <快照目录>       # 两级判定，见下
...
### `--verify` 的两级判定（2026-09-16 修正）

| 判定 | 含义 | 影响退出码 |
|---|---|---|
| **A. 快照自洽** | `files/` 里每个副本的 sha256 与 `manifest.json` 相符；补丁能套用到**快照记录的基线 commit** | ✅ 决定成败 |
| **B. 是否仍与当前工作区一致** | 还原到基线后的树 vs 当前工作区逐文件比对 | ❌ 只告警（过期信息） |

分开的理由：工作区后来又改过（甚至已提交），不该把一份**完好**的历史快照判成"损坏"；
反过来，副本真损坏时，也绝不能因为"工作区恰好也没改"而被掩盖。

> 🐞 **提交后跑 `--verify` 时暴露并修掉的两个真缺陷**：
> 1. 它用**当前 HEAD** 建树，却把 `manifest.head` 写进打印文案——"未提交"的年代两者恰好相同，
>    所以一直看不出来。提交后 HEAD 前进，输出变成"已从 HEAD (ca1cdf96) 导出"却导出了新 HEAD，
>    所有补丁都"套用失败"、哈希大面积不符，**看起来像快照坏了，实际是基线取错了**。
>    现改为按 `manifest.head` 建树并如实打印（当前 HEAD 不同时明确提示）。
> 2. **空补丁**（只含新增文件的快照，如刚提交完就建快照）会让 `git apply` 对空文件**报错** →
>    整份完好的快照被判"套用失败"。现已跳过并说明。
...
| 历史快照 vs 已提交的 HEAD（2026-09-16） | ✓ 补丁套用成功、自洽 `103/103`；⚠ 与当前工作区 23/103 不同（**正确地**判为"已过期"，退出码 0） |

**排除规则也要被测试**：备份工具最危险的失败模式是**排除过宽**——静默漏掉源码却仍然报绿。
`test-snapshot.mjs`（52 项）同时钉「该排除的排除」（尤其 `data/` 里的数据库与 API Key）
与「该包含的包含」，并对上面两级判定各做一条**阴性对照**
（只 B 不成立 → 退出码必须为 0；A 不成立 → 退出码必须为 1）；
另有排除规则的阴性对照：把前缀放宽成 `.p1-baseline/` → 测试失败 2 项。

> ⚠️ **快照只对生成那一刻成立**。改完代码要重新 `--create`；在提交过的仓库里，
> 旧的快照会（正确地）被报成"已过期"——它仍可还原到快照时点，但不再是当前状态的回滚点。
