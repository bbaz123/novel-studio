...
function traceUpsertOp(summary) {
  ...
  if (idx >= 0) {
    trace.ops[idx] = { ...trace.ops[idx], ...summary, summary, updatedAt: Date.now() };
  } else {
    trace.ops.unshift({ ...summary, summary, nodes: [], nodeCount: summary.nodes || 0, updatedAt: Date.now() });
    trace.ops = trace.ops.slice(0, 200);
  }
  traceMaybeRender();
}
...
function traceSessionTotals() {
  ...
  return { ops, nodes, errors, prompt, completion, aiCalls, truncated };
}
...
function renderTraceList() {
  ...
  if (statsEl) {
    statsEl.textContent = `${t.ops} 个操作 · ${t.nodes} 个节点 · ${t.errors} 个错误 · ${t.aiCalls} 次 AI · ${t.prompt + t.completion} tokens${t.truncated ? ` · ${t.truncated} 个操作被截断` : ''}`;
  }
  if (!ops.length) {
  ...
  try {
    ...
    if (serverOps.length) {
      for (const s of serverOps) {
        ...
        if (idx >= 0) {
          // 以服务端为准合并摘要；SSE 收到的节点保留在 nodes 里供展开。
          trace.ops[idx] = { ...trace.ops[idx], ...s, summary: s, updatedAt: Date.now() };
        } else {
          trace.ops.unshift({ ...s, summary: s, nodes: [], nodeCount: s.nodes || 0, updatedAt: Date.now() });
        }
      }
      trace.ops = trace.ops.slice(0, 200);
      renderTraceList();
