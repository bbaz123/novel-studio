...
run('装配回归：P1 基线 vs 当前（真实）', process.execPath,
  ...
  { requires: () => fs.existsSync('.p1-baseline/baselines-p5-real') });
...
run('装配回归：差异须恰好为已刻画的「压缩提示挪位」（压力）', process.execPath,
  ...
  { requires: () => fs.existsSync('.p1-baseline/baselines-pre-hint') && fs.existsSync('.p1-baseline/baselines-p5') });
...
run('压缩提示在层内（离线读压力基线）', process.execPath,
  ...
  { requires: () => fs.existsSync('.p1-baseline/baselines-p5') });
