 ...
 * 追踪专用时钟（单调、与系统时间调整无关）。
 *
 * ⚠️ 唯一时间出口：所有要把 `t0` 交给 traceFn / traceAI / traceHarness 的调用方
 * **必须**用本函数取值，禁止传 `Date.now()`。
 * 历史上 harness.js / server.js 传的是 `Date.now()`（Unix 纪元毫秒），与本函数的
 * `hrtime.bigint()`（开机以来毫秒）相减，得到 -1.787e12ms 量级的负数，
 * 让每一个 AI / 慢通道节点的耗时都不可读。
 */
...
/**
 * 追踪时钟自检：本函数返回的时钟与 Unix 纪元时钟（Date.now）相差多少毫秒。
 *
 * 用途：任何把 `Date.now()` 误传给追踪埋点的调用，都会让节点耗时变成这个量级的
 * 巨大负数（历史上出现过 -1.787e12ms）。启动时打印一次，可在事故之前就发现接线错误。
 */
export function traceClockDomain() {
  return { monotonic_ms: traceNow(), epoch_ms: Date.now(), drift_ms: Date.now() - traceNow() };
}

/** 节点耗时是否落在可信范围：超过一天即视为「另一个时钟域算出来的垃圾值」。 */
const COST_TRUST_MAX_MS = 24 * 60 * 60 * 1000;
...
function beginOp(opId, title, meta = {}, force = false) {
  ...
  const op = {
    ...
    status: 'running',
    httpStatus: null,
    // 空闲收尾用：最后一次「有新节点 / 请求有进展」的时刻，以及是否为自动收尾。
...
function newSessionId() {
  ...
  // 同秒内多次开始录制（停录→再开录）会撞名：旧实现只用 process.pid 做后缀，
  // 同一进程内两次会话的后缀完全相同，后一次会覆盖前一次的文件。
  // 加一段随机熵即可同时满足「同进程多次不撞」与「文件名可读」。
  const rand = Math.random().toString(36).slice(2, 8);
  return `${stamp}-${process.pid.toString(36)}${rand}`;
}
...
export function traceState() {
  return {
    ...
    errors: Array.from(state.ops.values()).reduce((s, op) => s + opErrorsOf(op), 0),
    totals: { ...state.totals },
...
/**
 * 一条操作的真实错误数：累加计数器只代表"建摘要那一刻"的事实，
 * 收尾之后补挂的 error 节点（客户端补报/迟到节点）不在其中。
 * 所有对外口径一律按节点事实重算，累加值只作下限 —— 否则会出现
 * 「有 error 节点、摘要却写 errors: 0」的自相矛盾记录（2026-09-14 事故）。
 */
function opErrorsOf(op) {
  let nodeErrors = 0;
  for (const n of op.nodes) {
    if (n && n.status === 'error') nodeErrors += 1;
  }
  return Math.max(op.errors, nodeErrors);
}
 ...
 * 结束一条操作。
 *
 * ⚠️ 幂等：已收尾的操作不会被再次收尾（重复 op-end 会让会话文件出现同一操作的
 * 多条摘要，回看时互相覆盖）。前端重复上报由调用方去重，这里再兜一层。
 *
 * @param {object} [extra] status/render/toast；`extra.auto` 表示这是追踪器主动补的收尾
 *   （操作空闲、客户端已断开），此类记录的耗时不代表业务真实耗时，摘要会带 auto_closed。
 */
export function finishOperation(opId, extra = {}) {
  const op = state.ops.get(sanitizeOpId(opId));
  if (!op) return null;
  if (op.status !== 'running') {
    // 已收尾：不重复写 op-end（重复摘要会让回看时前后覆盖）。
    // 但「追踪器自动补的收尾」不算最终结论 —— 客户端随后到达的显式状态（取消/失败）
    // 仍然要能被记录，否则用户主动取消会被 auto_closed: done 掩盖。
    if (op.autoClosed !== true || !extra.status || op.status === extra.status) return opSummary(op);
    op.status = String(extra.status).slice(0, 20);
    return opSummary(op);
  }
  op.costMs = Math.max(op.costMs, nowMs() - op.t0);
  op.endedAt = new Date().toISOString();
  op.status = extra.status ? String(extra.status).slice(0, 20) : (op.errors ? 'error' : 'done');
  if (extra.auto === true) op.autoClosed = true;
...
/** 空闲多久算「这条操作已经结束」。取值依据见 sweepIdleOperations。 */
const OP_IDLE_GRACE_MS = () => (Number(process.env.NOVELSTUDIO_TRACE_IDLE_MS) > 0
  ? Math.floor(Number(process.env.NOVELSTUDIO_TRACE_IDLE_MS))
  : 1200);

/**
 * 空闲收尾：把「请求都回完了、但没有任何人显式结束」的操作关掉。
 *
 * 为什么必须有：服务端自发的请求（无 X-Trace-Op 头）由 traceRequest 建记录，
 * 但产品里只有前端会调用 /api/debug/op 去结束操作。于是这类操作会一直挂在
 * 'running'，唯一的收尾者是停录时的兜底循环 —— 耗时被算到「停录那一刻」，
 * 30ms 的检索会被记成 546469ms。这里按「无在途请求 + 空闲超过宽限期」主动收尾，
 * 并把 auto_closed 置位，让界面能标出「这条耗时不可信」。
 *
 * 宽限期取 1200ms：前端操作收尾走 traceScheduleOpFlush（150ms）+ 一次 fetch，
 * 正常在数百毫秒内到达；给足余量避免与正常的客户端收尾抢跑。
 * 于是「响应结束 → 记录收尾」的误差约 1.2～1.4s，远小于真实业务操作（秒级～分钟级），
 * 相比旧行为（一直挂到停录，30ms 记成 546469ms）是数量级的修正。
 */
export function sweepIdleOperations(graceMs = OP_IDLE_GRACE_MS()) {
  if (!state.recording) return 0;
  const now = nowMs();
  let closed = 0;
  for (const op of state.ops.values()) {
    if (op.status !== 'running') continue;
    if ((op.pendingRequests || 0) > 0) continue;
    if (now - (op.lastActivityAt || op.t0) < graceMs) continue;
    finishOperation(op.opId, { auto: true });
    closed += 1;
  }
  return closed;
}
...
function opSummary(op) {
  let slowest = null;
  // 错误数/状态以「节点事实」重算：op.errors 是追加节点时累加的计数器，仅代表建记录
  // 那一刻的事实；客户端补挂的 error 节点、或收尾后迟到的 error 节点都不在其中。
  // 只读计数器会出现「有 error 节点，但摘要 errors: 0 / status: done」的自相矛盾记录。
  let nodeErrors = 0;
  let untrustedCost = false;
  for (const n of op.nodes) {
    if (n && n.status === 'error') nodeErrors += 1;
    const c = Number(n && n.cost_ms);
    if (Number.isFinite(c) && Math.abs(c) > COST_TRUST_MAX_MS) untrustedCost = true;
  }
  const errors = Math.max(op.errors, nodeErrors);
  let status = op.status;
  if (errors > 0 && status === 'done') status = 'error';
  // 自动收尾的「客户端已断开」请求：按 499 记，比 200 更接近事实。
  if (op.httpStatus === 499 && status === 'done') status = 'cancelled';
  for (const n of op.nodes) {
  ...
  return {
    ...
    // auto_closed：这条记录是「停录/空闲时补的收尾」，耗时与结束时间不可信，界面必须能区分。
    auto_closed: op.autoClosed === true,
    // cost_untrusted：操作内存在跨时钟域算出的垃圾耗时（|cost| > 1 天），
    // 此时摘要里的耗时/最慢节点都不能用于性能判断。
    cost_untrusted: untrustedCost,
...
export function traceRequest(req, res, fn) {
  ...
  return als.run(op, () => {
    ...
    res.on('close', () => {
      ...
      const code = aborted ? 499 : (res.statusCode || 499);
      op.httpStatus = Number.isFinite(op.httpStatus) ? Math.max(op.httpStatus, code) : code;
      op.lastActivityAt = nowMs();
    });
    return fn();
  });
