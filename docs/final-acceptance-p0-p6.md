...
### 补测（决策 D8-#1，2026-09-16 晚）：把"提交粒度"也验一遍

上面那句"回滚粒度 = 提交"**当时并没有被验证过**——它只是"阶段不可独立回滚"的推论。
按本项目「清单/判断类资产会静默腐烂、且不会自己报警」的教训，用 `git revert` 实测了一遍
（`.p1-baseline/revert-matrix.mjs`，在临时 worktree 里跑、不碰工作区；**自带阴性对照**：
先在临时仓库造一对必然冲突的提交，确认工具认得出冲突，才用它给结论）：

| 区间 | 可**单独**回滚 | 冲突 | 合计 |
|---|---|---|---|
| `ca1cdf96..1955cc01` | **4** | 10 | 14 |

冲突原因几乎都是"与后续提交改在同一批行"（`server.js` / `harness.js` / docs 被反复修改）。

**所以精确的说法要分两层**：

1. **外科式**撤销某一个提交、保留其后的提交 → 只有 **4/14** 可行；
2. **整体**回到历史上任意一点 → **永远可行**：
   `git reset --hard <sha>`（分支未合并，最直接；提交仍在 reflog 里可找回），
   或 `git checkout <sha>`（只想看看那时的状态）。

**实操建议**：要撤就撤**相邻的一整段**，不要从中间挑一个提交单独摘掉。
