...
**有一条硬约束值得记下**：`agent-default-model` 的 `reasoningEffort` **只存在于 dsh 的 settings 层、
且是进程级**，配置层带不了——所以切换任务级模型/强度目前只能改写 `~/.dsh/settings.yaml`，
这也正是 harness 任务必须**串行**的原因。
...
| 语义召回层在不可用时**整层消失** | 模型与作者都不知道「本该有一层召回但没来」 | 待处理 |
| 两条同步旁路（生成小说 / 记忆压缩）绕过作业设施 | 无进度、无取消、无落库、不计并发 | 待处理 |
| 并发上限与串行现实不一致 | 服务端允许 2 个作业，**界面路径实际吞吐 1** | **已可观测**（决策 D4，2026-09-16）：作业显示「等待模型槽位（前面还有 N 个任务）」、`/harness/status` 暴露 `model_load`；**吞吐未改**，`HARNESS_CONCURRENCY` 仍为 2（并行留给不请求模型/强度的任务） |
| 缓存命中时的陈旧窗口 | 上下文缓存有 120s TTL，极端情况下最多陈旧 2 分钟 | 已限定，未消除 |
