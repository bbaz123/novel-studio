...
| 缺口 | 影响 | 归属 |
|---|---|---|
| 长期记忆仍会随章节线性膨胀（`mergeMemoryDraft` 是文本拼接） | 中段内容进不了上下文（但可用 `novel_memory_read` 查回）；**压缩提示链路本身此前是断的**（F6：提示拼在层末尾、被自己截掉），已于 2026-09-15 修好；自动压缩仍靠手动/模型主动 | **护栏已就位**（D8-#3 前半，2026-09-16）：`ai/memory-compress-guard.mjs` 要求压缩结果保留全部角色与世界观实体，不通过就**拒绝落库**。两次真实调用实测：模型为"≤800 字"的任务实际产出 5.6k~22k 字（含推理，同样计费）。**覆盖率下限**是策略值，默认 1.0（一个都不许丢），可用 `NOVELSTUDIO_COMPRESS_MIN_COVERAGE` 放宽。**自动触发未做**，等策略确定 |
