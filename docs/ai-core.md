...
| 缺口 | 影响 | 归属 |
|---|---|---|
| 长期记忆仍会随章节线性膨胀（`mergeMemoryDraft` 是文本拼接） | 中段内容进不了上下文（但可用 `novel_memory_read` 查回）；压缩仍靠手动/模型主动 | 需付费许可才能做自动压缩 |
| 语义召回层在不可用时**整层消失** | 模型与作者都不知道「本该有一层召回但没来」 | 待处理 |
| 两条同步旁路（生成小说 / 记忆压缩）绕过作业设施 | 无进度、无取消、无落库、不计并发 | 待处理 |
| 并发上限与串行现实不一致 | 服务端允许 2 个作业，实际吞吐 1 | 待处理（改动牵扯批量流程） |
| 缓存命中时的陈旧窗口 | 上下文缓存有 120s TTL，极端情况下最多陈旧 2 分钟 | 已限定，未消除 |
| 记忆库内容缺失 | 部分作品的语义召回层长期为空（`ov_indexed_at` 却声称已索引） | 待用户决策是否重建 |
