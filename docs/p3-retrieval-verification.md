...
> 日期：2026-09-15　状态：**代码部分完成并实测通过**；记忆库索引缺失一项~~待你决策~~ **已于 2026-09-16 执行完毕（见下）**

> **2026-09-16 更新：P3-d 已执行，语义召回恢复。**
> 做法：`POST /api/novel/semantic_index`（按作品重建；省略 `work_id` 即对数据库里存在的
> 全部作品重建——范围由 `SELECT id FROM works` 决定，不是遍历目录）。
>
> | | 重建前 | 重建后 |
> |---|---|---|
> | work#2 召回 | `no-hits` | `ok`，6 条命中——`2/chapters/107.md`(91) · `2/characters/44.md`(86) · `2/meta.md`(83) · `2/long-memory.md`(82) 等，**全是真实内容文件**，不再被元数据噪声吃掉 |
> | work#9 召回 | `no-hits` | `ok`，8 条命中 |
> | 召回层在上下文里 | 整层不存在 | body=1759 / emitted=1400 / truncated=true |
> | work#2 上下文总量 | 6,710 字 | **8,169 字**（+1,459 = 召回层真正进入 prompt）|
>
> 各约 **30 秒**完成。work#2 的记忆库目录从"只剩骨架"变为完整内容
> （71 个文件：`chapters/107.md`、23 个 `characters/*.md`、31 个 `settings/*.md`、
> `events.md`、`long-memory.md`、`meta.md`、`outline.md`）。
>
> 顺带发现：记忆库下有 **185 个作品目录，而数据库只有 2 部作品** →
> **183 个孤儿目录**（2,022 个文件，分布在 09-06/09-13/09-14/09-15/09-16）。
> 处置见 `docs/pending-decisions.md` 的 **D7**。
