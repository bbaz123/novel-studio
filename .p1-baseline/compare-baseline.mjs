...
// ⚠️ 空输入必须**失败**，不能"通过"。实测过：两个空目录会打印
// `逐字节一致: 0/0` + `结论: IDENTICAL` 并 exit 0——**什么都没比，却报完全一致**。
// 这不是假想：逐用例基线 JSON 被 `.p1-baseline/.gitignore` 排除（可由 capture-baseline.mjs 重抓），
// 所以**新克隆的仓库里这些目录就是空的**，空转通过会让回归证据变成一句空话。
if (all.length === 0) {
  console.log('✗ 没有可比较的用例——不能判定"一致"。');
  console.log(`  目录：${dirA} / ${dirB}`);
  console.log('  常见原因：逐用例 JSON 未生成（被 .gitignore 排除、或没跑过 capture-baseline.mjs）。');
  console.log('  重抓：node .p1-baseline/capture-baseline.mjs --base <实例> --db <库> --out <目录>');
  process.exit(2);
}
