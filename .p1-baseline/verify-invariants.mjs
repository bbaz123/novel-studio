...
// ⚠️ 零用例必须**失败**，不能报"全部成立"。同 compare-baseline 的理由：
// 逐用例基线 JSON 被 `.p1-baseline/.gitignore` 排除，新克隆的仓库里目录是空的——
// 那时"用例数 0 → 没有违规 → ✓ 全部成立"就是把没检查说成通过。
if (checked === 0) {
  console.log('✗ 没有任何用例被校验——不能报"全部成立"。');
  console.log(`  目录：${dir}（匹配到 ${files.length} 个 JSON，其中 0 个可校验）`);
